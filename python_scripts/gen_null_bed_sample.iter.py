...
if __name__ == "__main__":
    ...
    for item in grab_cell_types:
        print("Working on %s cell type" % (item))
        grabbed_list = cell_type_split[item]
        cell_type_bed = pybedtools.BedTool(grabbed_list)
        cell_type_gc_content = capture_gc_content(cell_type_bed, args.gn)
        take_len = cell_type_bed.field_count()
        mean_cell_type_gc_score = calcualte_mean_GC_content(cell_type_gc_content, take_len)

        ## Generate matching list
        number_ACRs = cell_type_bed.count()

        print("Taking equal sample of Broad ACRs...")
        #matched_broad_acrs = generate_matching_GC(broadly_acc_acr_bed,
        #                                          number_ACRs, args.gn, mean_cell_type_gc_score, max_attempts = 1000)


        matched_broad_acrs = capture_regions_with_matching_GC(broadly_acc_acr_bed, 
                                     number_ACRs,
                                         args.gn, mean_cell_type_gc_score)


        output_file_name = args.o + "." + item + ".broad_acr_null_list.bed"
