...
def read_blast_file(input_file):
    ...
    with open(input_file, 'r') as f:
        for line in f:
            ...
            if cleaned_line[1] not in scaffold_with_hits_dict and float(cleaned_line[-2]) < .001 and float(cleaned_line[2]) > 80:
                scaffold_with_hits_dict[cleaned_line[1]] = [cleaned_line]
            elif cleaned_line[1] in scaffold_with_hits_dict and float(cleaned_line[-2]) < .001 and float(cleaned_line[2]) > 80:
...
def parse_blast_dict(blast_dict):
    ...
    finalized_dict = {}
    for chrom, blast_nested_list in blast_dict.items():
        finalized_dict[chrom] = []
        chrom_location_1_location_2 = chrom 
        ## qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore
        for blast_hit in blast_nested_list:
            #blastp_bed_regoins = generate_updated_coordinates(chrom_location_1_location_2, blast_hit)
            if int(blast_hit[8]) < int(blast_hit[9]):
                blastp_bed_regoins = [chrom, blast_hit[8], blast_hit[9]]
                finalized_dict[chrom].append(blastp_bed_regoins)

            elif int(blast_hit[8]) > int(blast_hit[9]):
                blastp_bed_regoins = [chrom, blast_hit[9], blast_hit[8]]
                finalized_dict[chrom].append(blastp_bed_regoins)
