...
final_annotation_set <- unlist(unique(pm_annotation_final$LouvainClusters))
names(umap_cluster_colors)  <- levels(final_annotation_set)
col_scale <- scale_colour_manual(name = "grp", values = umap_cluster_colors)


pm.louvain_clusters <- ggplot(pm_annotation_final, aes(x=umap1, y = umap2, color = LouvainClusters)) +
  theme_half_open() +
...
unique(pm_annotation_final$cell_type)

final_annotation_set <- unlist(unique(pm_annotation_final$cell_type))
names(umap_cluster_colors)  <- levels(final_annotation_set)
col_scale <- scale_colour_manual(name = "grp", values = umap_cluster_colors)


fig_1_umap_pdf <- ggplot(pm_annotation_final, aes(x = umap1, y = umap2, color = cell_type)) +
  theme_half_open() +
...
ggsave("pm.cell_annot.pdf", 
       plot = fig_1_umap_pdf, 
       path = "/Users/pablomendieta/Projects/comparative_single_cell/comparative_single_cell_imgs/figures/figure_1/UMAPs",
       units = "in",
       width = 8, height = 5)

ggsave("pm.cell_annot.png", 
       plot = fig_1_umap_pdf, 
       path = "/Users/pablomendieta/Projects/comparative_single_cell/comparative_single_cell_imgs/figures/figure_1/UMAPs",
       units = "in",
       width = 10, height = 10)
