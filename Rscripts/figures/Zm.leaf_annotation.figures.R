...
final_annotation_set <- unlist(unique(zm_annotation_final$LouvainClusters))
names(umap_cluster_colors)  <- levels(final_annotation_set)
col_scale <- scale_colour_manual(name = "grp", values = umap_cluster_colors)


zm.louvain_clusters <- ggplot(zm_annotation_final, aes(x=umap1, y = umap2, color = LouvainClusters)) +
  theme_half_open() +
...
final_annotation_set <- unlist(unique(zm_annotation_final$cell_type))
names(umap_cluster_colors)  <- levels(final_annotation_set)
col_scale <- scale_colour_manual(name = "grp", values = umap_cluster_colors)

unlist(unique(zm_annotation_final$cell_type))
...
fig_1_umap_pdf <- ggplot(zm_annotation_final, aes(x = umap1, y = umap2, color = cell_type)) +
  theme_half_open() +
...
ggsave("zm.cell_annot.pdf", plot = fig_1_umap_pdf, 
       path = "/Users/pablomendieta/Projects/comparative_single_cell/comparative_single_cell_imgs/figures/figure_1/UMAPs",
       units = "in",
       width = 8, height = 5)
